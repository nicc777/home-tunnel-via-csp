...
def from_ttl_event_get_stack_name(event: dict)->str:
    stack_name = None
    try:
        if event['command'] == 'delete_relay_server_stack':
            command_parameters = json.loads(event['command_parameters'])
            ...
            if 'parameter_name' not in command_parameters or 'parameter_value' not in command_parameters:
                logger.error('Expected keys in command parameters not found: {}'.format(event['command_parameters']))
            else:
                if command_parameters['parameter_name'] == 'stack_name':
                    stack_name = command_parameters['parameter_value']
        else:
            logger.error('Only supported command: "delete_relay_server_stack" - got "{}"'.format(event['command']))
    except:
        logger.error('Failed to get stack name from event: {}'.format(event))
        logger.debug('EXCEPTION: {}'.format(traceback.format_exc()))
...
def handler(event, context):
    ...
    if 'Records' in event:
        for record in event['Records']:
            ...
            # TODO - extract the stack name to delete...
    elif 'command' in event and 'command_parameters' in event:
        if event['command'] == 'delete_relay_server_stack':
            stack_name = from_ttl_event_get_stack_name(event=event)
